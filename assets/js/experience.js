(() => {
 ...
 function show(scroll){const model=models[Math.floor(index/3)];select(model,index%3);if(scroll)model.scrollIntoView({behavior:reduced.matches?'instant':'smooth',block:'start'});update();}
 ...
 function close(){running=false;clearTimeout(timer);dock.hidden=true;}
